...
def edit_meta(json_path: str | Path) -> bool:
    ...
    rewrite = "B0FieldSource" not in metadata or args.force_rewrite
    ...
    metadata["B0FieldSource"] = [
        f"{ses_id}pepolarb0" if bids_suffix == "dwi" else f"{ses_id}pepolarbold",
        f"{ses_id}phasediff",
    ]

    print(f"Rewriting after B0FieldSource metadata insertion: {json_path}.")
