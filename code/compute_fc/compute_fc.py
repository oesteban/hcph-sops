...
FIGURE_PATTERN = ['sub-{subject}/figures/sub-{subject}[_ses-{session}]'
                  '[_task-{task}][_meas-{meas}][_desc-{desc}]_{suffix}{extension}',
...
def get_arguments():
    ...
    parser.add_argument('--fc-estimator', default='sparse inverse covariance',
...
def get_func_filenames_bids(paths_to_func_dir, task_filter=[]):
    ...
    if len(unique_tr_s) > 1:
        logging.warning(f'Multiple TR values found, temporal filtering may not'
                        ' work as intended !')
...
def check_existing_output(output, func_filename, return_existing=False, **kwargs):
    ...
    missing_data_filter = [not op.exists(op.join(output, get_bids_savename(filename,
                                                                           **kwargs)))
...
def load_timeseries(func_filename, output):
    ...
    loaded_ts = []
    for filename in func_filename:
        path_to_ts = get_bids_savename(filename,
...
def extract_timeseries(func_filename, atlas_filename, masker=None, verbose=2, interpolate=False,
                       low_pass=None, t_r=None, **kwargs):
    ...
    #logging.debug(f'Denoising parameters are:\n', '\n\t'.join([f'{key}: {val}'
...
def compute_connectivity(time_series, strategy='sparse inverse covariance', verbose=False):
    n_ts = len(time_series)
    n_area = time_series[0].shape[-1]
    logging.info(f'Computing functional connectivity matrices for {n_ts} timeseries ...')
    ...
    connectivity_estimator = ConnectivityMeasure(cov_estimator=estimator, kind=connectivity_kind,
...
def plot_timeseries(timeseries, labels=None, normalize=False, plot_type='signal',
                    vert_scale=5, margin_value=.01):
    ...
    if 'carpet' in plot_type.lower():
        ...
        image = ax.imshow(timeseries_to_show.T, cmap='binary_r', aspect='auto',
...
def visual_report_timeserie(timeseries, filename, output, confounds=None, **kwargs):
    ...
    for plot_type, plot_desc in zip(['carpet', 'signal'], ['carpetplot', 'timeseries']):
        ts_saveloc = get_bids_savename(filename, patterns=FIGURE_PATTERN, desc=plot_desc,
                                       **FIGURE_FILLS)
        plot_timeseries(timeseries, plot_type=plot_type, **kwargs)

        logging.debug(f'Saving timeseries visual report at:')
    ...
    if confounds is not None:
        conf_saveloc = get_bids_savename(filename, patterns=FIGURE_PATTERN,
                                         desc='designmatrix', **FIGURE_FILLS)

        _, ax = plt.subplots(figsize=TS_FIGURE_SIZE)
        plot_design_matrix(confounds, ax=ax)
        logging.debug(f'Saving confounds visual report at:')
...
def visual_report_fc(matrix, filename, output, labels=None):
    fc_saveloc = get_bids_savename(filename, patterns=FIGURE_PATTERN, desc='heatmap',
                                   **FIGURE_FILLS)
    fig, ax = plt.subplots(figsize=FC_FIGURE_SIZE)

    plot_matrix(matrix, labels=list(labels), axes=ax, vmin=-1, vmax=1)
    
    logging.debug(f'Saving functional connectivity matrices visual report at:')
...
def main():
    ...
    if not overwrite:
        ...
        missing_ts, existing_ts = check_existing_output(output, func_filenames,
        ...
        missing_only_fc = check_existing_output(output, existing_ts, patterns=FC_PATTERN,
                                                **FC_FILLS)
    ...
    if save and len(time_series):
        logging.info(f'Saving timeseries ...')
        os.makedirs(output, exist_ok=True)
        save_output(time_series, missing_ts, output, patterns=TIMESERIES_PATTERN,
                    **TIMESERIES_FILLS)
    ...
    fc_matrices = compute_connectivity(time_series + existing_timeseries, strategy=fc_estimator)
    
    if save:
        logging.info(f'Saving connectivity matrices ...')
    ...
    logging.info(f'Computation is done for {len(missing_output)} files out of the '
