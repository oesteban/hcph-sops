...
def generate_synthetic_data(true_pi0, true_lambda, true_mu, true_sigma, n_samples, random_seed=None):
    """
...
def create_data_histogram(density_values, params, bins=50):
    """
    ...
    plt.hist(density_values, bins=bins, alpha=0.7, density=True)
    plt.axvline(x=params['mu'], color='red', linestyle='--', 
                label=f"True mean for connected: {params['mu']}")
    plt.title('Synthetic Density Data from Mixture Model')
    plt.xlabel('Density')
    plt.ylabel('Frequency')
    plt.legend()
    return fig

def fit_mixture_model(density_values, mu_type="fixed", mu_value=0.8, mu_prior_mean=0.8, mu_prior_sigma=0.2,
                     n_samples=2000, n_tune=1000, random_seed=None):
    """
    ...
    with pm.Model() as model:
        ...
        model_info = {
            'observed_data': density_values,
            'mu_type': mu_type,
            'mu_value': mu_value if mu_type == "fixed" else None
        }
        
        # Sample from the posterior
        trace = pm.sample(n_samples, tune=n_tune, target_accept=0.9, 
...
def create_analysis_plots(trace, params, model_info):
    ...
    param_comparison = {}
    for param_name, pymc_name in [("pi0", "pi0"), 
                                 ("lambda", "lambda_exp"), 
                                 ("sigma", "sigma"),
                                 ("mu", "mu")]:
    ...
    plt.hist(model_info['observed_data'], bins=50, 
            alpha=0.7, density=True, color='blue',
            label='Observed Data')
    
    sns.kdeplot(posterior_predictive.posterior_predictive["density"].values.flatten(),
                color='red', label='Posterior Predictive')
...
def run_simulation(true_pi0=0.3, true_lambda=2.0, true_mu=0.8, true_sigma=0.15, 
                  n_samples=2000, mu_type="fixed", mu_prior_mean=0.8, mu_prior_sigma=0.2,
                  random_seed=42, display_plots=False):
    """
    ...
    if display_plots:
        ...
        for param, values in analysis_results['param_comparison'].items():
            if 'note' in values:
                print(f"{param}: True = {values['true']:.3f}, Value = {values['estimated']:.3f} ({values['note']})")
            else:
                print(f"{param}: True = {values['true']:.3f}, Estimated = {values['estimated']:.3f}")
